...
- (void) doRecharge {
	...
	if ([[model currentPayVia] isEqualToString:@"Credit Card"]) {
		...
		NSString *inAppMsg = [NSString stringWithFormat:@"Do you want to buy one Reload Increment for %@?\n\n[Environment: Sandbox]", [model currentRechargeAmount]];
		
		UIAlertView *alert;
		alert = [[UIAlertView alloc] initWithTitle:@"Confirm Your In App Purchase" message:inAppMsg delegate:self cancelButtonTitle:@"Cancel" otherButtonTitles:@"Buy", nil];
		[alert setTag:2];
		[alert show];
		[alert release];
...
- (void)alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex {
	...
	if ([alertView tag] == 1) {
		...
		if (buttonIndex == 0) {
			...
			// do stuff
			[mainDelegate rechargeViaInAppDemoWithAmount:[model currentRechargeAmount]];
			[activity stopAnimating];
			[activity setHidden:YES];
			
			
			NSString *successMsg = [NSString stringWithFormat:@"Your purchase was successful.\n\n[Environment: Sandbox]", [model currentRechargeAmount]];
			
			UIAlertView *alert2;
			alert2 = [[UIAlertView alloc] initWithTitle:@"Thank You" message:successMsg delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil];
			[alert2 setTag:3];
			[alert2 show];
			[alert2 release];
